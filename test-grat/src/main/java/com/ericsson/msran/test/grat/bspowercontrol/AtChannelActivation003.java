...
import com.ericsson.abisco.clientlib.AbiscoServer.Routing;
	...
	public class AtChannelActivation003 extends TestBase {
	    /**
	    ...
	    public void bsPowerAtChannelActivation003(String testId, String description) throws InterruptedException,
	            JSONException {
	        ...
	        // 02.01.003 Channel Activation TCH
	        //
	        ...
	        power[0] = TRAFFRN.Enums.BSPower.Pn;      // power level for TS 0
	        power[1] = TRAFFRN.Enums.BSPower.Pn_2dB;  // power level for TS 1
	        power[2] = TRAFFRN.Enums.BSPower.Pn_4dB;  // power level for TS 2
	        power[3] = TRAFFRN.Enums.BSPower.Pn_8dB;  // power level for TS 3
	        power[4] = TRAFFRN.Enums.BSPower.Pn_14dB; // power level for TS 4
	        power[5] = TRAFFRN.Enums.BSPower.Pn_16dB; // power level for TS 5
	        power[6] = TRAFFRN.Enums.BSPower.Pn_18dB; // power level for TS 6
	        power[7] = TRAFFRN.Enums.BSPower.Pn_20dB; // power level for TS 7

	        expectedPowerLevel[0] = 0;
	        expectedPowerLevel[1] = -2;  // should be = powerLevel[0] - 2
	        expectedPowerLevel[2] = -4;  // should be = powerLevel[0] - 4
	        expectedPowerLevel[3] = -8;  // should be = powerLevel[0] - 8
	        expectedPowerLevel[4] = -14; // should be = powerLevel[0] - 14
	        expectedPowerLevel[5] = -16; // should be = powerLevel[0] - 16
	        expectedPowerLevel[6] = -18; // should be = powerLevel[0] - 18
	        expectedPowerLevel[7] = -20; // should be = powerLevel[0] - 20

	        // CC_1
	        // Setup channels on TS 0 - 7.
	        setTestStepBegin("02.01.003 Channel Activation TCH, CC_1");

	        for (int ts = 0; ts < 8; ts++) // Loop for each time slot
	        {
	            ccId[ts] = setupChannel(trxc, CC_1, (short)ts, msId[ts], power[ts]);
	        }
	        ...
	        for (int ts = 0; ts < 8; ts++) // Loop for each time slot
	        {
	            ...
	            int avgRfl = avgRfls.get(ts);
	            powerLevel[ts] = avgRfl / 256; // unit is 1/256 dBm
	            setTestInfo("avgRfl for TS " + ts + " = " + avgRfl + ", power level = " + powerLevel[ts] + " (avgRfl / 256)");

	            // Verify power levels.
	            if (powerLevel[ts] - powerLevel[0] - expectedPowerLevel[ts] > 1) // powerLevel[0] used as reference
	            {
	                fail("Difference between measured and expected values");
	            }

	            // Release channel.
	            releaseChannel(trxc, CC_1, (short)ts, ccId[ts]);
	        }
