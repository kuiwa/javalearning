...
import com.ericsson.abisco.clientlib.servers.TRAFFRN.ModeModify;
...
public class LinkEstablishIndicationConEst extends TestBase {
	...
	@TestInfo(
            tcId = "LTE00230716",
            slogan = "Link Establish Indication Contention Establishment",
            ...
            requirementIds = { "04.08:0001" },
            verificationStatement = "Ongoing",
            testDescription = "Verifies Link Establish Indication Contention Establishment.",
            traceGuidelines = "N/A")
     ...
     * @name LinkEstablishIndication
     * 		
     * @description Verify Link Establish Indication Contention Establishment.
     *
    ...
    public void linkEstablishIndicationConEstSubTc1Test(String testId, String description) throws InterruptedException, JSONException {
		    		    	
		// SubTest case 1, Contention Resolution case
		...
		// Send modeModify from ABISCO
		modeModify();
		
		// Send SABM on SAPI3 from MSSIM
		SAPI = 3;
		csDataReqSapi3();    
		
		// Verify that Establish Indication has been sent from BTS to BSC on ABIS 
		// on SAPI 3
		setTestInfo("ABISCO: Waiting for EstablishIndication on SAPI 3 for subTc1 "); 
		expSapi = 3;
		expLinkId = LinkId.SACCH;
		verifyEsInd(establishIndicationQueue);
		establishIndicationQueue.clear();
	...
	@Test (timeOut = 1800000)
    @Parameters({ "testId", "description" })
    public void linkEstablishIndicationConEstSubTc2Test(String testId, String description) throws InterruptedException, JSONException {
		// SubTest case 2, Contention Resolution case
		// Tested channel Combination		Chan (SAPI=0)		Chan (SAPI=3)		ChnComb						ChnMain
		...
		// Verify that Establish Indication has been sent from BTS to BSC on ABIS 
		// on SAPI 3
		setTestInfo("ABISCO: Waiting for EstablishIndication on SAPI 3 for subTc2 ");
		expSapi = 3;
		expLinkId = LinkId.SACCH;
		verifyEsInd(establishIndicationQueue);
		establishIndicationQueue.clear();
	...
	@Test (timeOut = 1800000)
    @Parameters({ "testId", "description" })
    public void linkEstablishIndicationConEstSubTc3Test(String testId, String description) throws InterruptedException, JSONException {
	...
	@Test (timeOut = 1800000)
    @Parameters({ "testId", "description" })
    public void linkEstablishIndicationConEstSubTc4Test(String testId, String description) throws InterruptedException, JSONException {
		...
		if (TypOfBCCH == BCCHType.NCOMB)
		{
			...
			setTestInfo("SAPI:3 is not applicable for subTc4");
			
			// Cleanup
			setTestStepBegin("Cleanup");
			//abisHelper.radioChannelsReleaseCommand();    
			// rFChannelRelease
			subTc = 4;
			rFChannelRelease();
	...
	@Test (timeOut = 1800000)
    @Parameters({ "testId", "description" })
    public void linkEstablishIndicationConEstSubTc5Test(String testId, String description) throws InterruptedException, JSONException {
		...
		// Verify that Establish Indication has been sent from BTS to BSC on ABIS 
		// on SAPI 3
		setTestInfo("ABISCO: Waiting for EstablishIndication on SAPI 3 for subTc5 ");
		expSapi = 3;
		expLinkId = LinkId.SACCH;
		verifyEsInd(establishIndicationQueue);
		establishIndicationQueue.clear();
     ...
     * @name csDataReqSapi3
     * 
     * @description Send MSSIM.gsmphMPH_CS_DATA_REQ  on Link id SAPI=3
     *
     */
    private void csDataReqSapi3() throws InterruptedException, JSONException {
    	
    	Chan chan = null;
    	
    	// Set logical channel    	
    	if (subTc == 1 || subTc == 2 || subTc == 5 )
        {
    		chan = Chan.GSMPH_SACCH;
        }
        else 
        {
        	chan = Chan.GSMPH_SDCCH;
    	...
    	if (subTc == 1 || subTc == 2 || subTc == 5 )
    	{
    		// Send GsmphMPH_CS_DATA_REQ second time 
    		GsmphMPH_CS_DATA_REQ cs_data_REQ2 = GsmbFactory.getGsmphMPH_CS_DATA_REQBuilder(CcId).timeout(20)
    			.chan(chan) 
    			.burst(Burst.NORMAL_BURST)
    			.eaId((short) 0)
    			.spare((short) 0)
    			// For SAPI=3 
    			.data(new short[] {0, 0, 13, 63, 1, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43, 43})
    			//.data(new short[] {1, 63, 45, 5, 8, 112, 0, 0, 0, 0, 0, 0, 1, 0, 43, 43, 43, 43, 43, 43, 43, 43, 43})
    			.build();            
    		Confirmation confirmation2 = gsmb.send(cs_data_REQ2);
    		assertEquals("gsmphMPH_CS_DATA_REQ confirmation error 2", PhErrorType.GSM_PH_ENOERR, confirmation2.getErrorType());
    	}
    	else
    	{
    		fail("Not applicable for subTc3 and subTc4");
    	}
  ...
  //MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
     ...
     * @name modeModify
     * 
     * @description Send Mode Modify (TypeOfCh = SPEECH, ChannelRate = x, AlgOrRate = GSM1 )
     *
     */
    private void  modeModify() throws InterruptedException {
    	
        setTestInfo("Send Mode modify");
        TRAFFRN traffrn = abisHelper.getRslServer();        
        ModeModify modeModify = traffrn.createModeModify();
        ...
        ChannelModeStruct channelModeStruct_mode = new ChannelModeStruct();
        channelModeStruct_mode.setDTXUplink(DTXUplink.Off);
        channelModeStruct_mode.setDTXDownlink(DTXDownlink.Off);
        channelModeStruct_mode.setReserved(0);
        channelModeStruct_mode.setTypeOfCh(TypeOfCh.SPEECH);
        channelModeStruct_mode.setAlgOrRate(AlgOrRate.GSM1);
        
        if (subTc == 1)
        {
        	//Channel Combination (i)
        	channelNoStruct.setChannelType(ChannelType.Bm); 
        	channelModeStruct_mode.setChannelRate(ChannelRate.Bm); 
        }
        else if (subTc == 2)
        {     
        	//Channel Combination (ii)
        	channelNoStruct.setChannelType(ChannelType.Lm_0); 
        	channelModeStruct_mode.setChannelRate(ChannelRate.Lm);
        ...
        else 
        {
        	//Channel Combination (viii)
        	channelNoStruct.setChannelType(ChannelType.Bm); 
        	channelModeStruct_mode.setChannelRate(ChannelRate.Bm_BI); 
        }
        
        modeModify.setRouting(routing);
        modeModify.setChannelNoStruct(channelNoStruct);
        modeModify.setChannelModeStruct(channelModeStruct_mode);
        ...
        setTestInfo("Mode modify has been sent xxx");
      
        DISPATCH dispatch_mode = abisHelper.getDISPATCH();
        DISPATCH.SendStoredMessages sendStoredMessages_mode = dispatch_mode.createSendStoredMessages();
        sendStoredMessages_mode.setIssueUNACK(DISPATCH.Enums.IssueUNACK.No);
        sendStoredMessages_mode.sendAsync();
        
        setTestInfo("Mode modify has been sent");
    }
    //MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
    /**
     * @throws InterruptedException 
