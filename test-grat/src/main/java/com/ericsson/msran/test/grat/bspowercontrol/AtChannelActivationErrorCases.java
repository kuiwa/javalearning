...
public class AtChannelActivationErrorCases extends TestBase {
    ...
    int arfcn = 0;
    List<Integer> EXP_RECEIVE_DATA_PN16 = Arrays.asList(8,35,1,11,26,3,104,4,16);
    List<Integer> EXP_RECEIVE_DATA_PN31 = Arrays.asList(8,35,1,12,26,3,104,4,31);
    ...
    public void atChannelActivationErrorCasesTest(String testId, String description) throws InterruptedException, JSONException {
        ...
        //Send Channel activation on the bts with TS = 3, BS Power = Pn-16
        sendData(16);
        
      //Send Channel activation on the bts with TS = 3, BS Power = Pn-31
        sendData(31);
    ...
    private void  sendData(int pn) throws InterruptedException {
    	setTestInfo("Sending sendData::channelActivationNormalAssign with Pn: " +pn);        
       ...
       LAPD lapd = new LAPD();
        ...
        if (pn == 16){
        	//11=TS3,
        	bitstream.setData(Arrays.asList(8,33,1,11,3,1,6,4,0,1,8,1,4,16,13,0,24,0));
        }else if (pn == 31){
        	//12=TS4
        	bitstream.setData(Arrays.asList(8,33,1,12,3,1,6,4,0,1,8,1,4,31,13,0,24,0));
        }else{
        ...
        VerifyResponseData(response, pn);
   }
        
        private void  VerifyResponseData(ReceiveData response, int pn) throws InterruptedException {
        	setTestInfo("Sending rfChannelRelease on Abis");
        	if (pn == 16){
            	if (response.getData().equals(EXP_RECEIVE_DATA_PN16)){
            		setTestDebug("Correct data received " + response.getData());
            	}else{
            		fail("Wrong data received " + response.getData());
            	}
            }else if(pn == 31){
            	if (response.getData().equals(EXP_RECEIVE_DATA_PN31)){
            		setTestDebug("Correct data received " + response.getData());
            	}else{
            		fail("Wrong data received " + response.getData());
            	}
            }else{
            	fail("Could not verify response data");
            }
          }
